(defproject <<name>> "0.1.0-SNAPSHOT"
  ...
  :min-lein-version "<<min-lein-version>>"<% if resource-paths %>
  ...
  <<clean-targets>><% endif %><% if cljs %><% if shadow-cljs %>
  :shadow-cljs
  <<shadow-cljs-config>>
  :npm-deps [<<npm-deps>>]
  :npm-dev-deps <<npm-dev-deps>><% else %>
  :figwheel <<figwheel>><% endif %><% endif %>
...
<% if not shadow-cljs %>             :cljsbuild <% endif %><<uberjar-cljsbuild>><% endif %>
   ...
   :project/dev  {:jvm-opts ["-Dconf=dev-config.edn"<% for opt in opts %> <<opt>><% endfor %>]
                  ...
                  :plugins      [<<dev-plugins>>]<% if cljs %><% if not shadow-cljs %>
                  :cljsbuild <% endif %><<dev-cljsbuild>><% endif %><% if cljs-test %><% if not shadow-cljs %>
   ...
   :project/test {:jvm-opts ["-Dconf=test-config.edn"<% for opt in opts %> <<opt>><% endfor %>]
                  :resource-paths ["env/test/resources"]<% if cljs %><% if not shadow-cljs %>
                  :cljsbuild <% endif %><<test-cljsbuild>><% endif %>}
   :profiles/dev {}
   :profiles/test {}})
